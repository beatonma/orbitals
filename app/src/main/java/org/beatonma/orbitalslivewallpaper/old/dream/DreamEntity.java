...
public class DreamEntity {
    private final static String TAG = "DreamEntity";
    private DreamView game;
    ...
    private boolean explodingAnyway = false;
    ...
    // Constructors
    public DreamEntity(DreamView g, int x, int y, double dx, double dy,
    ...
    public int getColor() {
        ...
        renderColor = Color.argb(game.getObjectAlpha(), r1, g1, b1);
    ...
    public void doGravity(DreamEntity other) {
        // F_g = G((m1*m2)/(r^2))
        // a = F/m
        ...
        if (cdiff > 0) { // positive x distance
            ...
            this.dx += Math.cos(Math.atan(grad)) * a; // calculate horizontal
                                                        // component of
                                                        // acceleration
            ...
            this.dx -= Math.cos(Math.atan(grad)) * a;

        }

        this.dy += Math.sin(Math.atan(grad)) * a; // calculate vertical
                                                    // component of acceleration

//        this.dx *= getSpeedMultiplier();
//        this.dy *= getSpeedMultiplier();
    }

    public boolean detectCollision(DreamEntity other) {
    ...
    public void plot(float ff) {
        ...
        alpha = (int) Utils.map(alpha, 0, 255, 0, game.getColorAlpha());
        paint.setAlpha(alpha);
        //float halfDifference = difference * 0.5f;
        //path.lineTo((int) (x + ((Math.random() * difference) - halfDifference)) * ff, (int) (y + ((Math.random() * difference) - halfDifference)) * ff);
    ...
    public void doCollision(DreamEntity other) {
    ...
    public void collapseToBlackHole(DreamEntity e) {
        e.radius = 10;
        e.isBlackHole = true;
    ...
    public void setCollisionDelay(int n) {
        allowCollisionDelay = n;
    }
    ...
    public void weWereExplodingAnyway() {
        explodingAnyway = true;
    }

    public boolean wereWeExplodingAnyway() {
        return explodingAnyway;
    }
